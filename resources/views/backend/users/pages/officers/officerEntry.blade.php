...
@section('title',Auth::user()->name.'-officer-entry')
...
@section('content')
    <div class="ml-auto mr-auto col-sm-8 ">
        ...
        <div class="card-body">
            <form action="{{ URL::to('/user/officer') }}" method="post"  enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label for="name"> Name:<span class="ml-3" id="name"><b style="color: red">@error('name')*{{ $message }}@enderror</b></span></label>
                    <input type="text" class="form-control" value="{{ old('name') }}" id="name" placeholder="Enter officer name" name="name">
                </div>
                <div class="form-group">
                    ...
                    <input class="form-control" id="designation" name="designation" placeholder="Enter officer designation">
                </div>
                <div class="form-group">
                    <label for="phone">Phone :<span class="ml-3" id="phone"><b style="color: red">@error('phone')*{{ $message }}@enderror</b></span></label>
                    <input class="form-control"  id="phone" name="phone" placeholder="Enter officer phone no.">
                </div>
                <div class="form-group">
                    <label for="email">Email :<span class="ml-3" id="email"><b style="color: red">@error('email')*{{ $message }}@enderror</b></span></label>
                    <input class="form-control"  id="email" name="email" placeholder="Enter officer email">
                </div>
                <div class="form-group">
                    <style>
