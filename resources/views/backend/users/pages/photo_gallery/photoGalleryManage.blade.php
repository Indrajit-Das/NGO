...
@section('title',Auth::user()->name.'-manage-photo')
                    ...
                    @foreach($photoGallery as $key=>$photo)
                        <tr>
                           <style>
                            ...
                            <td>{{ Str::limit($photo->description, 25),' (...)' }}</td>
                             <td>
                            ...
                            <td>
                                <form action="{{ URL::to('user/photo-gallery/'.$photo->id) }}" method="post" onSubmit="return confirm('Do you want to delete photo?')" class="admin">
                                 <a href="{{ URL::to('user/photo-gallery/'. $photo->id.'/edit') }}" class="ml-1" title="Edit"><i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i></a>
                                    @csrf
                                    @method('DELETE')
